/obj/effect/proc_holder/magic/architect
...
/obj/effect/proc_holder/magic/architect/check_turf_cast(turf/target)
...
/obj/effect/proc_holder/magic/architect/check_object_cast(obj/target)
...
/obj/effect/proc_holder/magic/architect/cast_on_turf(turf/target)
...
/obj/effect/proc_holder/magic/architect/cast_on_object(turf/target)
	var/datum/effect/effect/system/steam_spread/steam = new /datum/effect/effect/system/steam_spread()
	steam.set_up(6, 0, target)
	steam.start()
	if(owner.current.a_intent == "hurt")
		message_admins("[usr] ([usr.ckey]) deleted a turf at X=[target.x] Y=[target.y] Z=[target.z] with [src.name] spell.(<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[usr.x];Y=[usr.y];Z=[usr.z]'>JMP</a>)")
		qdel(target)
