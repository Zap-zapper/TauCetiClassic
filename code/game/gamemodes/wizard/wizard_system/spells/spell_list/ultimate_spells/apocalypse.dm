/obj/effect/proc_holder/magic/pandemonium
	name = "Apocalypse"
	desc = ""
	mana_cost = 0
	ultimate = TRUE
...
/obj/effect/proc_holder/magic/pandemonium/cast_on_mob(mob/living/target)
	playsound(target.loc, 'sound/magic/monsters/monster1.ogg', 100, 0,channel = 1)
	playsound(target.loc, 'sound/magic/monsters/monster2.ogg', 100, 0,channel = 2)
	playsound(target.loc, 'sound/magic/monsters/monster3.ogg', 100, 0,channel = 3)
	playsound(target.loc, 'sound/magic/monsters/monster4.ogg', 100, 0,channel = 4)
	for(var/mob/M in range(10,target))
		if(M.client)
			shake_camera(M, 17, 2)
	for(var/turf/simulated/T in range(10,target))
